...
impl<'a, H: DeviceHal, U: UserInteraction> DeviceLoop<'a, H, U> {
    ...
    /// Hold the device on whatever the UI is showing until it is power cycled,
    /// keeping the screen alive. Used when an upgrade is refused: the active slot
    /// is untouched so the running firmware is still good, but resuming the loop
    /// would leave the coordinator and the chain in a half-finished upgrade
    /// conversation.
