...
pub enum FirmwareAction {
    ...
    ResetRequested,
    /// The image was downloaded but refused, so the active slot is untouched and
    /// the running firmware is still good. Resuming the loop would leave the
    /// coordinator and the chain in a half-finished upgrade conversation, so the
    /// device parks on the reason screen until it is power cycled.
    UpgradeRefused,
}
