...
fn ordinary_send_hides_the_single_change_output() {
    ...
    assert_eq!(prompt.self_payment, None);
    assert_eq!(prompt.value_moved(), Amount::from_sat(60_000));
}

#[test]
fn foreign_only_has_no_self_screen() {
    let mut template = template_with_input(100_000);
...
#[test]
fn send_that_also_pays_our_own_external_address_discloses_it() {
    let mut template = template_with_input(100_000);
    ...
    let prompt = prompt(&template);
    assert_eq!(prompt.self_payment, Some(Amount::from_sat(30_000)));
...
fn pure_self_spend_discloses_everything_including_change() {
    ...
    let prompt = prompt(&template);
    assert!(prompt.foreign_recipients.is_empty());
    assert_eq!(prompt.self_payment, Some(Amount::from_sat(99_000)));
    assert_eq!(prompt.value_moved(), Amount::from_sat(99_000));
}
...
fn two_change_outputs_with_a_foreign_recipient_are_disclosed() {
    ...
    let prompt = prompt(&template);
    assert_eq!(prompt.self_payment, Some(Amount::from_sat(49_000)));
    assert_eq!(prompt.value_moved(), Amount::from_sat(99_000));
}
