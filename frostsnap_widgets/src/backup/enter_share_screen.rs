...
impl EnterShareScreen {
    ...
    /// Whether the widgets have caught up with the model: no deferred
    /// keyboard/selector swap pending and no tap fade still playing — the
    /// same condition [`draw`](Widget::draw) requires before applying the
    /// deferred update. Until this is true the on-screen surface can be STALE
    /// relative to [`view_state`](Self::view_state): drags are dropped while
    /// an outgoing word selector is still up, taps are swallowed, and input
    /// aimed at the new surface can land on the old one. A driver must gate
    /// each action on it.
    pub fn is_settled(&self) -> bool {
        !self.pending_model_update && self.touches.is_empty()
    }
