...
pub mod electrum;
pub use electrum::{Door, ElectrumServers, IdentitySpec, Mode, Shape};
...
pub struct Regtest {
    ...
    /// The two controllable endpoints the APP reaches electrs through. The harness's own
    /// queries (`electrsd.client`) deliberately bypass them, so shutting a door for a test
    /// never blinds the faucet.
    electrum: ElectrumServers,
}
...
impl Regtest {
    ...
    pub fn start() -> anyhow::Result<Self> {
        ...
        let electrs_port: u16 = electrsd
            .electrum_url
            .rsplit(':')
            .next()
            .and_then(|port| port.parse().ok())
            .ok_or_else(|| anyhow::anyhow!("no port in electrs url `{}`", electrsd.electrum_url))?;
        let electrum =
            ElectrumServers::start(SocketAddr::from((Ipv4Addr::LOCALHOST, electrs_port)))
                .context("start the electrum front doors")?;
    ...
    /// The electrum URL to point the app's PRIMARY slot at — front door `a`. Dynamic, so it is
    /// seeded into the app rather than relied on as a default.
    pub fn electrum_url(&self) -> String {
        self.electrum.primary().url()
    }

    /// The URL for the app's BACKUP slot — front door `b`. A different host from
    /// [`electrum_url`](Self::electrum_url) (`localhost` against `127.0.0.1`) so the two get
    /// separate TOFU trust entries.
    pub fn electrum_backup_url(&self) -> String {
        self.electrum.backup().url()
    }

    /// The controllable front doors, for the `electrum_*` control verbs.
    pub fn electrum(&self) -> &ElectrumServers {
        &self.electrum
    }
