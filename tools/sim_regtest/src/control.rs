...
//! ```text
//!   {"cmd":"electrum_url"}                  -> {"ok":true,"url":"tcp://127.0.0.1:PORT"}  (front door `a`)
...
//!   {"cmd":"watch_descriptor","descriptor":"tr(..)#cks"} -> {"ok":true}
//!   {"cmd":"rpc","method":"<name>","params":[..]} -> {"ok":true,"result":<json>}   (raw bitcoind RPC)
//!   {"cmd":"create_psbt","address":"bcrt1..","sats":N}   -> {"ok":true,"psbt":"<base64>"}
//!   {"cmd":"ping"}                          -> {"ok":true,"pid":PID}
//!   {"cmd":"down"}                          -> {"ok":true,"down":true}   (then serve returns)
//! ```
//!
//! The electrum front doors (see [`crate::electrum`]) — the app's two "servers":
//! ```text
//!   {"cmd":"electrum_list"}
//!     -> {"ok":true,"servers":[{"id":"a","url":"tcp://127.0.0.1:P","mode":"tcp",
//!                               "identity":null,"fingerprint":null,"connections":0,
//!                               "accepted":0}, ..]}
//!   {"cmd":"electrum_set_mode","id":"a","mode":"tcp"|"down"|"hang"}   -> {"ok":true,"url":".."}
//!   {"cmd":"electrum_set_mode","id":"a","mode":"tls","identity":"a1"
//!    [,"shape":"normal"|"expired"|"wrong_host"|"signed_by"][,"issuer":"a1"]}
//!                                                                    -> {"ok":true,"url":"ssl://.."}
//!   {"cmd":"electrum_drop_connections","id":"a"}         -> {"ok":true,"dropped":N}
//! ```
//! `identity` NAMES a certificate; `shape` says how to mint it the first time that name is
//! used and is an error if the name already exists as something else. The url comes back
//! because the scheme changes with the mode, and the test needs the exact string to type into
//! the app's Edit Server dialog. `down` drops connections rather than refusing them — a door
//! never releases its port, see [`crate::electrum::accept_loop`].
...
use crate::{IdentitySpec, Mode, Regtest, Shape};
    ...
    let result: anyhow::Result<(Value, bool)> = (|| match req.get("cmd").and_then(Value::as_str) {
        ...
        Some("rpc") => {
            let method = req
                .get("method")
                .and_then(Value::as_str)
                .ok_or_else(|| anyhow!("missing/invalid string field `method`"))?;
            let params = match req.get("params") {
                None | Some(Value::Null) => Vec::new(),
                Some(Value::Array(params)) => params.clone(),
                Some(_) => return Err(anyhow!("`params` must be an array")),
            };
            Ok((
                json!({"ok": true, "result": regtest.rpc(method, &params)?}),
                false,
            ))
        }
        Some("watch_descriptor") => {
            let descriptor = req
                .get("descriptor")
                .and_then(Value::as_str)
                .ok_or_else(|| anyhow!("missing/invalid string field `descriptor`"))?;
            regtest.watch_descriptor(descriptor)?;
            Ok((json!({"ok": true}), false))
        }
        Some("create_psbt") => {
            let address = req
            ...
            let sats = req
                ...
                .ok_or_else(|| anyhow!("missing/invalid integer field `sats`"))?;
            Ok((
                json!({"ok": true, "psbt": regtest.create_psbt(address, sats)?}),
                false,
            ))
        }
        Some("electrum_list") => {
            let servers: Vec<Value> = regtest
                .electrum()
                .doors()
                .iter()
                .map(|door| {
                    let identity = door.identity();
                    json!({
                        "id": door.id(),
                        "url": door.url(),
                        "mode": door.mode().as_str(),
                        "identity": identity.as_ref().map(|(name, _)| name.clone()),
                        "fingerprint": identity.as_ref().map(|(_, fp)| fp.clone()),
                        "connections": door.connection_count(),
                        "accepted": door.accepted_count(),
                    })
                })
                .collect();
            Ok((json!({"ok": true, "servers": servers}), false))
        }
        Some("electrum_set_mode") => {
            let door = regtest.electrum().door(str_field(&req, "id")?)?;
            let mode = str_field(&req, "mode")?;
            let (mode, spec) = match mode {
                "tcp" => (Mode::Tcp, None),
                "down" => (Mode::Down, None),
                "hang" => (Mode::Hang, None),
                "tls" => {
                    let name = str_field(&req, "identity")
                        .context("mode `tls` needs an `identity` to present")?
                        .to_string();
                    let shape = match req.get("shape").and_then(Value::as_str) {
                        Some(shape) => Shape::parse(shape)?,
                        None => Shape::Normal,
                    };
                    let issuer = req.get("issuer").and_then(Value::as_str).map(str::to_owned);
                    (
                        Mode::Tls(name.clone()),
                        Some(IdentitySpec {
                            name,
                ...
                other => return Err(anyhow!("unknown electrum mode `{other}`")),
            };
            Ok((
                json!({"ok": true, "url": door.set_mode(mode, spec)?}),
                false,
            ))
        }
        Some("electrum_drop_connections") => {
            let door = regtest.electrum().door(str_field(&req, "id")?)?;
            Ok((
                json!({"ok": true, "dropped": door.drop_connections()}),
                false,
            ))
        }
...
/// A required string field, named in the error so a malformed request says which one.
fn str_field<'a>(req: &'a Value, name: &str) -> anyhow::Result<&'a str> {
    req.get(name)
        .and_then(Value::as_str)
        .ok_or_else(|| anyhow!("missing/invalid string field `{name}`"))
}
...
mod tests {
    ...
    fn faucet_lib_and_control_socket() {
        ...
        // ---- the front door is transparent to a REAL electrum client ----
        // The fast tests in `electrum` prove the modes against an echo backend; only this one
        // proves a door in front of actual electrs still speaks electrum. Both doors, because
        // `localhost` and `127.0.0.1` are separate listeners.
        for door in regtest.electrum().doors() {
            let url = door.url();
            let authority = url.split_once("://").unwrap().1;
            let through_door = electrsd::electrum_client::raw_client::RawClient::new(
                authority,
                Some(Duration::from_secs(10)),
            )
            .unwrap_or_else(|e| panic!("electrum client via door `{}` ({url}): {e}", door.id()));
            assert_eq!(
                through_door
                    .block_headers_subscribe()
                    .unwrap_or_else(|e| panic!("headers via door `{}`: {e}", door.id()))
                    .height,
                regtest.electrs_tip_height().expect("tip"),
                "door `{}` must report the same tip as electrs itself",
                door.id()
            );
        }
        ...
        let client = {
            ...
            thread::spawn(move || -> Vec<Value> {
                ...
                vec![
                    ...
                    call(r#"{"cmd":"rpc","method":"getblockcount","params":[]}"#),
                    call(r#"{"cmd":"rpc","method":"definitelynotanrpc"}"#),
                    call(r#"{"cmd":"electrum_list"}"#),
                    call(r#"{"cmd":"electrum_set_mode","id":"b","mode":"tls","identity":"b1"}"#),
                    call(r#"{"cmd":"electrum_list"}"#),
                    call(r#"{"cmd":"electrum_drop_connections","id":"a"}"#),
                    call(r#"{"cmd":"electrum_set_mode","id":"a","mode":"nonsense"}"#),
                    call(r#"{"cmd":"nope"}"#),
                    call(r#"{"cmd":"down"}"#),
                ]
            })
        ...
        // The passthrough's envelope must carry a NON-OBJECT result verbatim…
        assert_eq!(replies[7]["ok"], json!(true), "rpc: {:?}", replies[7]);
        assert!(
            replies[7]["result"].as_u64().unwrap() >= 103,
            "rpc getblockcount returns the bare number: {:?}",
            replies[7]
        );
        // …and a Core-side failure must surface as the normal error reply.
        assert_eq!(
            replies[8]["ok"],
            json!(false),
            "unknown rpc method is a clean error: {:?}",
            replies[8]
        );
        assert!(
            replies[8]["error"]
                .as_str()
                .unwrap()
                .contains("definitelynotanrpc"),
            "the error names the failing method: {:?}",
            replies[8]
        );
        // Both doors are listed, `a` first, each with the url the app would be pointed at.
        let servers = replies[9]["servers"].as_array().expect("electrum_list");
        assert_eq!(servers.len(), 2, "two front doors: {:?}", replies[9]);
        assert_eq!(servers[0]["id"], json!("a"));
        assert_eq!(servers[1]["id"], json!("b"));
        assert_eq!(servers[0]["mode"], json!("tcp"));
        assert_eq!(
            servers[0]["fingerprint"],
            json!(null),
            "a plaintext door presents no certificate"
        );
        // Serving TLS changes the scheme and starts reporting the identity, so a test can
        // assert the app was shown THIS certificate.
        assert!(
            replies[10]["url"].as_str().unwrap().starts_with("ssl://"),
            "tls url: {:?}",
            replies[10]
        );
        let after = replies[11]["servers"].as_array().unwrap();
        assert_eq!(after[1]["mode"], json!("tls"));
        assert_eq!(after[1]["identity"], json!("b1"));
        assert_eq!(
            after[1]["fingerprint"].as_str().map(str::len),
            Some(64),
            "fingerprint is a hex sha256: {:?}",
            after[1]
        );
        assert_eq!(after[0]["mode"], json!("tcp"), "door `a` is untouched");
        assert!(
            replies[12]["dropped"].as_u64().is_some(),
            "drop_connections reports a count: {:?}",
            replies[12]
        );
        assert_eq!(
            replies[13]["ok"],
            json!(false),
            "an unknown mode is a clean error, not a door left half-changed: {:?}",
            replies[13]
        );
        assert_eq!(
            replies[14]["ok"],
        ...
        assert_eq!(replies[15]["down"], json!(true), "down");
