...
pub use backup_typist::{
    next_step, record_on_device, type_into_screen, type_on_device, BackupTarget, EntryMode,
    EntryStep, EntryView, TypistError,
};
pub use chain_router::{ChainRouter, SlotError, SlotSpec};
pub use clock::SimClock;
pub use device::{DeviceSurfaces, SimUi, VirtualDevice, VirtualDeviceSession};
pub use display::{FramebufferDisplay, SharedFramebuffer, HEIGHT, WIDTH};
pub use firmware::{FirmwareDigestCell, SimFirmware};
pub use flash::{RamFlash, SECTORS};
pub use hal::{SimDownstream, SimHal, SimUpstream};
pub use identity::DeviceIdentityCell;
pub use input::DeviceInput;
pub use observation::{EntryOutcome, EntryProgress, SimObservation};
pub use saved_state::DeviceSavedState;
...
mod tests {
    ...
    // Fresh-header entropy must not replay across PROCESS launches. The coordinator's DB
    // outlives its process, so if a relaunched sim rebuilt a blank device from the same
    // seed and drew the same entropy, that device would reclaim the previous launch's id
    // and the app would re-attach its old name and wallet — the identity resurrection the
    // per-boot RNG exists to remove. Two epochs stand in for two launches; the boot
    // counter is 0 in both, exactly as it is after a restart.
    #[test]
    fn blank_header_identity_differs_across_process_epochs() {
        let derive = |epoch: [u8; 32], nonce: u64| {
            let flash = std::cell::RefCell::new(RamFlash::new());
            let mut nvs = FlashPartition::new(&flash, 0, SECTORS as u32, "nvs");
            let header_flash = FlashHeader::new(nvs.split_off_front(2));
            let header = header_flash.init(&mut crate::device::boot_rng_from(7, epoch, nonce));
            header
                .device_keypair(&mut SimKeyedHash::from_seed(7, "fixed-entropy-key"))
                .public_key()
        };
        ...
        assert_ne!(
            derive([1u8; 32], 0),
            derive([2u8; 32], 0),
            "a new process launch must not re-mint the previous launch's device id"
        );
        assert_ne!(
            derive([1u8; 32], 0),
            derive([1u8; 32], 1),
            "within a launch, each boot draws its own entropy"
        );
        assert_eq!(
            derive([1u8; 32], 0),
            derive([1u8; 32], 0),
            "the draw is a pure function of its two entropy inputs"
        );
    }
    ...
    fn one_virtual_device_registers_through_the_coordinator() {
        ...
        // nothing in this scenario upgrades firmware, so no offer may occur.
        drop(session);
        assert_eq!(
            device.upgrades_offered(),
            0,
            "an up-to-date device must not be offered a firmware upgrade"
        );
    }
    ...
    fn run_one_device_keygen(seed: u64) -> (VirtualDevice, frostsnap_core::AccessStructureRef) {
        let (device, asr, ()) = run_one_device_keygen_then(seed, |_| ());
        (device, asr)
    }

    /// The live pieces of a completed keygen, handed to a `run_one_device_keygen_then`
    /// body: the device is still booted and still wired to the coordinator, so a test can
    /// drive a further protocol flow (erase) against a device that really holds a share.
    struct KeygenRig<'a, 'd> {
        session: &'a mut VirtualDeviceSession<'d>,
        coordinator: &'a mut SimCoordinator,
        touch: &'a TouchQueue,
        framebuffer: &'a SharedFramebuffer,
        device_id: frostsnap_core::DeviceId,
        asr: frostsnap_core::AccessStructureRef,
    }

    fn run_one_device_keygen_then<R>(
        seed: u64,
        body: impl FnOnce(KeygenRig<'_, '_>) -> R,
    ) -> (VirtualDevice, frostsnap_core::AccessStructureRef, R) {
    ...
    // The keygen completes without any firmware-upgrade offer.
    #[test]
    ...
    // The saved-state codec preserves a keygen share end to end: run a REAL keygen,
    // pocket the device through DeviceSavedState bytes (the drawer), boot a device from
    // the decoded state — it still holds the key. The restore path the router uses
    // is this same from_saved construction; its identity/digest semantics are pinned
    // at the router level.
    #[test]
    fn saved_state_codec_preserves_a_keygen_share() {
        let (device, asr) = run_one_device_keygen(3);
        let bytes = DeviceSavedState {
            seed: 3,
            digest: SimFirmware::PLACEHOLDER_DIGEST,
            flash: device.into_flash(),
        }
        .to_bytes();

        let decoded = DeviceSavedState::from_bytes(&bytes).unwrap();
        let mut restored = VirtualDevice::from_saved(
            decoded.seed,
            FirmwareDigestCell::new(decoded.digest),
        ...
        let session = match restored.session() {
            InitOutcome::Ready(session) => session,
            InitOutcome::ResetRequested => panic!("saved-state boot should be Ready"),
        };
        assert!(
            session.holds_key(asr.key_id),
            "the share must survive the saved-state bytes"
        );
    }

    // The ERASE flow end to end at the leaf, against a device that really holds a share:
    // the coordinator sends DataErase, the user confirms on the device (swipe up, then
    // the long hold), the device wipes NVS and resets — and what boots back is a
    // DIFFERENT device with no share. Identity dying with the flash header is the whole
    // point: while a wiped device re-minted its OLD id, the app re-attached its name and
    // wallet on the re-announce and erasing looked like it did nothing.
    #[test]
    fn erase_wipes_the_share_and_mints_a_new_identity() {
        use frostsnap_comms::{CoordinatorSendBody, CoordinatorSendMessage};
        use std::time::{Duration, Instant};

        // The erase prompt pages with a swipe-up, then holds far longer than keygen's
        // (a destructive action): drive the real widgets, not an injected UiEvent.
        const CONFIRM_POINT: embedded_graphics::geometry::Point =
            embedded_graphics::geometry::Point::new(120, 215);
        let seed = 7;

        let (device, asr, (id_before, confirmations)) = run_one_device_keygen_then(seed, |rig| {
            assert!(
                rig.session.holds_key(rig.asr.key_id),
                "the device must hold a share before we erase it"
            );
            rig.coordinator
                .usb_sender()
            ...
            let pump = |rig_session: &mut VirtualDeviceSession<'_>,
                        coordinator: &mut SimCoordinator,
                        how_long: Duration| {
                let until = Instant::now() + how_long;
                while Instant::now() < until {
                    lockstep_step_device(rig_session);
                    coordinator.step();
                    std::thread::sleep(Duration::from_millis(15));
                }
            };
            // Let the warning page render, then page to the hold screen.
            pump(rig.session, rig.coordinator, Duration::from_secs(2));
            save_phase_png(rig.framebuffer, "frostsnap_erase_prompt.png");
            DeviceInput::new(rig.touch.clone()).swipe(
                120,
                240,
                120,
                80,
            ...
            pump(rig.session, rig.coordinator, Duration::from_secs(1));

            // Hold until the device asks the shell to reset — the erase ran to
            // completion. `EraseConfirmed` (a Misc message) lands mid-hold: it is the
            // device telling the coordinator destruction has BEGUN.
            let mut reset = false;
            let mut confirmations = 0usize;
            let until = Instant::now() + Duration::from_secs(60);
            while Instant::now() < until && !reset {
                hold_to_confirm(rig.touch, CONFIRM_POINT);
                reset = lockstep_step_device(rig.session);
                confirmations += rig.coordinator.step().ignored_misc;
                std::thread::sleep(Duration::from_millis(15));
            }
            assert!(reset, "a confirmed erase must drive the device to reset");
            (rig.session.device_id(), confirmations)
        });

        assert!(
            confirmations >= 1,
            "the device must tell the coordinator the erase started (EraseConfirmed)"
        );

        // The reboot the shell performs after the reset: same flash, wiped.
        let mut rebooted = VirtualDevice::from_saved(
            seed,
            FirmwareDigestCell::new(SimFirmware::PLACEHOLDER_DIGEST),
            PipeByteIo::disconnected(),
            PipeByteIo::disconnected(),
            DeviceSurfaces::new(),
            device.into_flash(),
        ...
        let session = match rebooted.session() {
            InitOutcome::Ready(session) => session,
            InitOutcome::ResetRequested => panic!("the erased device should boot blank, not reset"),
        };
        assert!(
            !session.holds_key(asr.key_id),
            "an erased device must not hold the share"
        );
        assert_ne!(
            session.device_id(),
            id_before,
            "an erased device is a NEW device: the header its id derives from was wiped"
        );
    }

    // Identity is a property of the FLASH, so the paths that carry the flash carry the
    // identity: a power-cycle and a saved-state restore both come back as the SAME
    // device. (The erase test above pins the other half — no flash header, new device.)
    #[test]
    fn identity_survives_a_power_cycle_and_a_restore() {
        let seed = 11;
        let (device, _asr) = run_one_device_keygen(seed);

        // Boot a flash, read the identity it yields, and hand the flash back.
        let boot = |flash: RamFlash| -> (frostsnap_core::DeviceId, RamFlash) {
            let mut booted = VirtualDevice::from_saved(
                seed,
                FirmwareDigestCell::new(SimFirmware::PLACEHOLDER_DIGEST),
                PipeByteIo::disconnected(),
                PipeByteIo::disconnected(),
            ...
            let id = match booted.session() {
                InitOutcome::Ready(session) => session.device_id(),
                InitOutcome::ResetRequested => panic!("boot from preserved flash should be Ready"),
            };
            (id, booted.into_flash())
        };

        let (id, flash) = boot(device.into_flash());
        let (power_cycled, flash) = boot(flash);
        assert_eq!(
            power_cycled, id,
            "a power-cycle preserves identity (the header is in the flash)"
        );
        ...
        .to_bytes();
        let decoded = DeviceSavedState::from_bytes(&bytes).unwrap();
        let (restored, _) = boot(decoded.flash);
        assert_eq!(
            restored, id,
            "a restored device is the same device (the header travelled with the flash)"
        );
    }
    ...
    // empty flash — does NOT, proving the check is about a runtime-written value
    // surviving rather than anything the seed alone reproduces.
    #[test]
