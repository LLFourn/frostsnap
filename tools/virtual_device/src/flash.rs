...
impl RamFlash {
    ...
    /// The raw NVS content — the device's durable bytes, for snapshotting.
    pub fn as_bytes(&self) -> &[u8] {
        &self.0[..]
    }

    /// Rebuild a flash from snapshotted bytes; the length must be exactly the
    /// fixed esp NVS geometry this store models.
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, String> {
        let content: Box<[u8; CAPACITY]> =
            bytes.to_vec().into_boxed_slice().try_into().map_err(|_| {
                format!(
                    "flash snapshot must be exactly {CAPACITY} bytes, got {}",
                    bytes.len()
                )
            })?;
        Ok(Self(content))
