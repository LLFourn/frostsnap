...
impl DeviceInput {
    ...
    /// Advance the paged backup display one page: an up-swipe whose span comes
    /// from the display widget's own geometry contract
    /// ([`BackupDisplay::page_advance_swipe`]) — callers never supply pixels.
    pub fn backup_display_next(&self) {
        let (from, to) = frostsnap_widgets::backup::BackupDisplay::page_advance_swipe(
            embedded_graphics::geometry::Size::new(crate::WIDTH, crate::HEIGHT),
        );
        self.swipe(from.x, from.y, to.x, to.y, Duration::from_millis(150));
    }

    /// Hold the backup display's confirmation control long enough to confirm
    /// (`BackupRecorded`); the point is probed from the confirmation page's
    /// own hit-testing ([`BackupDisplay::confirm_point`]).
    pub fn backup_display_confirm(&self) {
        let p = frostsnap_widgets::backup::BackupDisplay::confirm_point(
            embedded_graphics::geometry::Size::new(crate::WIDTH, crate::HEIGHT),
        )
        .expect("confirmation page has a hold button");
        // Hold time + the button's fade-in + margin.
        let hold = frostsnap_widgets::HOLD_TO_CONFIRM_TIME_MS as u64 + 600;
        self.hold(p.x, p.y, Duration::from_millis(hold));
    }
