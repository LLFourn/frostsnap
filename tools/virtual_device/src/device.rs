...
use frostsnap_embedded::{
    ...
    DownstreamConnectionState, FlashPartition, KeyedHash, ShareEncryptionSecrets,
...
/// Counts boots within this process.
static BOOT_NONCE: AtomicU64 = AtomicU64::new(0);
/// This process's slice of OS entropy, drawn once. The counter alone restarts at zero
/// in the next process, and the coordinator's DB OUTLIVES the process — so a relaunched
/// app rebuilding a blank device from the same sim seed would reclaim the previous
/// launch's device id and resurrect exactly the identity this models away.
static BOOT_EPOCH: OnceLock<[u8; 32]> = OnceLock::new();

fn process_epoch() -> [u8; 32] {
...
/// The RNG a booting device gets. The esp RNG is hardware-backed, so the entropy a
/// header init consumes is never repeated — not across boots, and not across power
/// cycles of the whole bench. This is load-bearing for identity: `FlashHeader::init`
/// draws the device-id seed from here, so a stream that ever replays mints a device
/// id that already existed. (The efuse-analog keys stay seed-derived: those are the
/// chip, not the flash.)
fn boot_rng(seed: u64) -> ChaCha20Rng {
    boot_rng_from(
        seed,
        process_epoch(),
...
/// [`boot_rng`] with its two entropy inputs explicit — the seam the cross-launch
/// regression drives, where two epochs stand in for two process launches.
pub(crate) fn boot_rng_from(seed: u64, epoch: [u8; 32], nonce: u64) -> ChaCha20Rng {
    let mut hasher = SimKeyedHash::from_seed(seed, "boot-rng");
    let mut input = [0u8; 40];
    input[..32].copy_from_slice(&epoch);
    input[32..].copy_from_slice(&nonce.to_le_bytes());
    ChaCha20Rng::from_seed(hasher.keyed_hash("frostsnap-sim-boot", &input))
}
...
impl VirtualDevice {
    /// Build a device. `seed` fixes the efuse-analog dev keys (the chip's identity
    /// half); the device's ID additionally depends on the flash header, so a
    /// blank-flash boot mints a fresh one (see [`boot_rng`]). The upstream link is a
    /// connected pipe (its coordinator end is [`VirtualDevice::host_serial`], driven
    /// in sim-2); the downstream link has no peer.
