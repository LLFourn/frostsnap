...
pub struct StepOutcome {
    ...
    /// Device ids that asked for a name and were sent their name preview this step.
    pub named: Vec<DeviceId>,
    ...
    pub fn step(&mut self) -> StepOutcome {
        ...
        for change in self.manager.poll_ports() {
            match change {
                ...
                // Send the device its name preview, mirroring the app (wallet-create
                // sends `update_name_preview` as the user types; the standalone
                // rename/confirm flow is retired). The device stages it as the pending
                // name that keygen finalize requires (an unnamed device panics on
                // FinalizeKeyGen) and persists + reports it (SetName) at finalize.
                DeviceChange::NeedsName { id } => {
                    self.usb_sender.update_name_preview(id, self.device_name());
                    outcome.named.push(id);
                }
