...
pub fn type_on_device(
    ...
    const ATTEMPTS: usize = 5;
    /// Wait after a key visibly lands before the next swipe+tap. The model
    /// updates at lift-up, but the WIDGETS lag it (`pending_model_update`
    /// until the ~100 ms tap fade + a ≤25 ms draw): drags sent while a stale
    /// word selector is still up are dropped, so a clamp swipe straddling
    /// that window applies only partially and the follow-up tap can land
    /// mid-scroll on an unintended (valid!) key, silently desyncing the
    /// device from the target. Letting the transition finish closes that
    /// window; sized from the widgets' own animation constants.
    const POST_ACTION_SETTLE: Duration = Duration::from_millis(250);
    ...
    loop {
        ...
        std::thread::sleep(POST_ACTION_SETTLE);
    }
}
