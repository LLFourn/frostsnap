...
use crate::identity::DeviceIdentityCell;
...
/// is volatile (rebuilt each power-on). The device id lives in the flash header, so it
/// survives a power-cycle and changes when an erase wipes that header — hence the
/// [`DeviceIdentityCell`] rather than a captured value.
struct DeviceSlot {
    seed: u64,
    ...
    /// The device's LIVE identity (see [`DeviceIdentityCell`]): every boot publishes
    /// into it, so an erase — which re-inits the flash header in place — is reflected
    /// here rather than leaving the slot labelling a wiped device as its old self.
    identity: DeviceIdentityCell,
    power: Power,
}

impl DeviceSlot {
    /// This slot's identity as of its latest boot. Published before the spawn returns,
    /// so a slot always has one.
    fn live_identity(&self) -> DeviceId {
        self.identity
            .get()
            .expect("a spawned slot has published its identity")
    }
...
impl ChainRouter {
    ...
    /// one — the restore-from-saved-state construction (the flash carries the header, so
    /// the restored device has the same identity, shares intact).
    fn build_slot_from(spec: SlotSpec, flash: RamFlash) -> (DeviceSlot, DeviceLink) {
        ...
        let identity = DeviceIdentityCell::new();
        let thread = spawn_device_thread(
            spec.seed,
            digest.clone(),
            identity.clone(),
    ...
    /// publish at a fresh index. The check matters for RESTORES: the same saved state
    /// carries the same flash header, so restoring it twice would put one device in the
    /// pool twice — a duplicate in every coordinator map keyed by identity. (A fresh add
    /// mints its identity from a per-boot RNG, so it cannot collide by construction.)
    fn insert_slot(&self, mut slot: DeviceSlot, link: DeviceLink) -> Result<usize, SlotError> {
        slot.power_off();
        let new_id = slot.live_identity();
        let mut slots = self.slots.lock().unwrap();
        if slots
            .iter()
            .flatten()
            .any(|existing| existing.live_identity() == new_id)
        {
    ...
    /// Device `index`'s CURRENT id. Stable across power-cycles and saved-state restores
    /// (both carry the flash header it derives from) and NEW after an erase, which
    /// re-inits that header — so callers must read it live rather than cache it.
    pub fn device_id(&self, index: usize) -> Result<DeviceId, SlotError> {
        self.with_live_slot(index, |slot| slot.live_identity())
    }
...
mod tests {
    ...
    fn router_chain_registers_then_reconfigures_over_one_port() {
        ...
        let ids: Vec<DeviceId> = (0..3).map(|i| router.device_id(i).unwrap()).collect();
        assert_eq!(
    ...
    fn add_device_grows_fleet_and_connects_at_tail() {
        ...
        let mut ids: Vec<DeviceId> = (0..2).map(|i| router.device_id(i).unwrap()).collect();
        ...
        let new_id = router.device_id(new_index).unwrap();
    ...
    fn disconnect_powers_off_and_reconnect_drives_through_the_same_handle() {
        ...
        let device_id = router.device_id(0).unwrap();
    ...
    fn remove_device_tombstones_the_index_and_keeps_numbering_stable() {
        ...
        assert_eq!(router.device_id(2).unwrap(), ids[2]);
        ...
        assert!(
            !ids.contains(&router.device_id(new_index).unwrap()),
    ...
    // SAME identity (the flash carries its header) and the set digest; and a saved state whose
    ...
    fn save_and_restore_preserve_identity_and_reject_duplicates() {
        ...
        let id1 = router.device_id(1).unwrap();
        ...
        assert_eq!(
            router.device_id(restored).unwrap(),
            id1,
            "the restored flash carries the header: the same device identity"
        );
    ...
    // The THREADED erase, end to end through the router: a device is erased in place
    // (the slot keeps running — the thread reboots inside it), so every layer that
    // reports identity has to follow the reboot. This drives the load-bearing path the
    // hand-built reboot in the lib.rs erase test cannot reach: the second boot's
    // `identity.publish`, the slot's cell, `ChainRouter::device_id`, and `insert_slot`'s
    // uniqueness check — a stale publish here would leave the router (and the tray and
    // the harness above it) naming a wiped device as its old self while the leaf tests
    // still passed.
    //
    // The final move is the acceptance the whole model rests on: the PRE-erase saved
    // state restores into the SAME pool while the erased slot is still live. It can only
    // succeed because the erased slot no longer holds that identity — the drawer copy is
    // the device the pool just lost.
    #[test]
    fn an_erased_slot_frees_its_identity_for_the_pre_erase_state() {
        // The erase prompt pages with a swipe-up and then holds; both are the real
        // widgets, driven through the real touch surface.
        const CONFIRM: (i32, i32) = (120, 215);

        let coord = host();
        let serial = VirtualSerial::single("sim-device-0", coord.clone());
        let port = serial.connection();
        let router = ChainRouter::new(coord, port, specs(1, 300), vec![0]);
        let mut manager = UsbSerialManager::new(Box::new(serial));

        // The coordinator's own view of who is on the wire — a factory-fresh device
        // announces (Connected) without being named, and that announce is what the app
        // keys every device record by.
        let announced = |manager: &mut UsbSerialManager, want: &dyn Fn(DeviceId) -> bool| {
            let deadline = Instant::now() + Duration::from_secs(30);
            while Instant::now() < deadline {
                for change in manager.poll_ports() {
                    if let DeviceChange::Connected { id, .. } = change {
                        if want(id) {
                            return Some(id);
                        }
                    }
        ...
        let id_before = router.device_id(0).unwrap();
        assert_eq!(
            announced(&mut manager, &|id| id == id_before),
            Some(id_before),
            "the device should announce itself before we erase it"
        );

        // Pocket a copy of the device as it is now (save needs it disconnected), then
        // plug it back in and erase THAT device.
        router.disconnect(0).unwrap();
        let pocketed = router.save_device_state(0).unwrap().to_bytes();
        router.connect(0).unwrap();
        // The coordinator only routes to devices it can currently SEE (a message for an
        // absent device is dropped, not queued), so wait for the replug's re-announce
        // before telling it to erase.
        assert_eq!(
            announced(&mut manager, &|id| id == id_before),
            Some(id_before),
            "the device should re-announce after being plugged back in"
        );
        ...
        // Erasing is destructive, so its confirm hold is far longer than keygen's
        // (the screen asks for 8 seconds) and `hold` RELEASES at the end — a short
        // hold resets the widget's progress instead of accumulating it. Each attempt
        // pages up first, so an attempt that raced the prompt's first render simply
        // retries rather than holding on the warning page forever.
        const ERASE_HOLD: Duration = Duration::from_millis(9500);
        let input = DeviceInput::new(router.touch(0).unwrap());
        // Pump the link so the DataErase actually goes out and the prompt renders.
        {
            let until = Instant::now() + Duration::from_millis(1500);
            while Instant::now() < until {
                let _ = manager.poll_ports();
                std::thread::sleep(Duration::from_millis(10));
            }
        }
        let deadline = Instant::now() + Duration::from_secs(120);
        while Instant::now() < deadline && router.device_id(0).unwrap() == id_before {
            input.swipe(120, 240, 120, 80, Duration::from_millis(250));
            std::thread::sleep(Duration::from_millis(200));
            input.hold(CONFIRM.0, CONFIRM.1, ERASE_HOLD);
            let _ = manager.poll_ports();
        }

        let id_after = router.device_id(0).unwrap();
        assert_ne!(
            id_after, id_before,
            "the slot must report the identity of the device that booted after the erase"
        );
        // ...and the coordinator meets that device as a STRANGER: a new id on the wire,
        // which is why nothing it knew about the erased device can re-attach.
        assert_eq!(
            announced(&mut manager, &|id| id != id_before),
            Some(id_after),
            "the erased device must re-announce under its new identity"
        );

        // The pocketed copy goes back in beside the erased slot: one device per identity
        // still holds, and the identity it wants is free precisely because the erase took
        // it from slot 0.
        let decoded = crate::DeviceSavedState::from_bytes(&pocketed).unwrap();
        let noop: FrameSink = Arc::new(|_, _, _| {});
        let restored = router
            .add_device_from_saved_state(decoded, noop)
            .expect("the pre-erase identity is free once the erase took it");
        assert!(router.is_live(0), "the erased slot is still in the pool");
        assert_eq!(
            router.device_id(restored).unwrap(),
            id_before,
            "the drawer copy is the device the pool lost"
        );
    }

    // Identity comes from the FLASH HEADER, minted per boot — so a blank device is
    // never a duplicate of anything, whatever seed it is built with (the seed only
    // fixes the efuse-analog keys). Two blank adds sharing a seed are two distinct
    // devices, exactly as two freshly-flashed boards are.
    #[test]
    fn blank_devices_are_distinct_even_with_the_same_seed() {
        let coord = host();
        let serial = VirtualSerial::single("sim-device-0", coord.clone());
        let port = serial.connection();
        ...
        let noop: FrameSink = Arc::new(|_, _, _| {});
        // "Another session's" BLANK device, imported at the seed a fresh add also uses.
        ...
        let fresh = router
            ...
            .expect("a blank device cannot collide with anything");

        let ids = [
            router.device_id(0).unwrap(),
            router.device_id(restored).unwrap(),
            router.device_id(fresh).unwrap(),
        ];
        assert_eq!(
            ids.iter().collect::<std::collections::HashSet<_>>().len(),
            3,
            "every blank boot mints its own identity: {ids:?}"
        );
    }

    // The uniqueness guard's real job: the SAME saved state carries the same flash
    // header, so restoring it twice would put one device in the pool twice — a
    // duplicate in every coordinator map keyed by identity.
    #[test]
    fn the_same_saved_state_cannot_be_restored_twice() {
        let coord = host();
        let serial = VirtualSerial::single("sim-device-0", coord.clone());
        let port = serial.connection();
        let router = ChainRouter::new(coord, port, specs(1, 262), vec![]);

        let saved = router.save_device_state(0).unwrap();
        let bytes = saved.to_bytes();
        let noop: FrameSink = Arc::new(|_, _, _| {});
        let id0 = router.device_id(0).unwrap();

        let decoded = crate::DeviceSavedState::from_bytes(&bytes).unwrap();
        assert_eq!(
            router
                .add_device_from_saved_state(decoded, noop.clone())
                .unwrap_err(),
            SlotError::IdentityInPool { id: id0 },
            "the original is still in the pool"
        );

        // Once the original leaves the pool, its identity is free again.
        router.remove_device(0).unwrap();
        let decoded = crate::DeviceSavedState::from_bytes(&bytes).unwrap();
        let restored = router
            .add_device_from_saved_state(decoded, noop)
            .expect("the identity is no longer live");
        assert_eq!(router.device_id(restored).unwrap(), id0);
    }
    ...
    fn remove_racing_reconnect_cannot_republish_the_device() {
        ...
        router.set_chain(vec![]).unwrap();
        router.set_chain(vec![0]).unwrap();
        let _ = router.device_id(0).unwrap();
    }
