...
//! lost, NVS kept.
//!
//! The thread runs `loop { poll_once(); short park }` — frame EXPORT rides the UI's
//! own poll (see `ObservedUi::poll`), so the screen keeps streaming even when a
//! blocking takeover (the firmware-upgrade drain) owns the loop. `poll_once` is
//! non-blocking and the park is short, so the loop checks the stop flag every
//! iteration; [`DeviceThread::power_off`] (and `Drop`) set the flag and join
//! promptly (no detached thread, no busy spin).

use crate::device::{DeviceSurfaces, VirtualDevice};
...
pub struct DeviceHandles {
    ...
    /// The sim's read-only screen observation; the thread publishes into it
    /// and clears it on exit (reset / power-off).
...
struct ClearOnExit(SimObservation);
...
impl DeviceThread {
    /// Power the device off: signal stop, join the thread, and return its preserved
    /// flash (NVS). The volatile loop/UI/RAM is gone. The firmware digest needs no
    /// hand-back — it lives in the slot's shared [`FirmwareDigestCell`], which a
    /// completed upgrade already wrote in-thread.
...
/// the device has booted and published its id into `identity` — EVERY boot publishes, so a
/// reboot that re-inits the flash header (the erase path) is seen by whoever holds the cell.
pub(crate) fn spawn_device_thread(
    ...
    let join = thread::spawn(move || -> RamFlash {
        ...
        // Each iteration is one BOOT. `Poll::ResetRequested` (an upgrade's
        // reset, or the init-time recovery erase) reboots here — same flash,
        // same digest cell — instead of leaving a dead thread in a slot that
        // claims to be powered.
        loop {
            // Built here so nothing `!Send` (FrostyUi/the session) ever escapes the thread.
            let mut device = VirtualDevice::from_saved(
                seed,
                firmware_digest.clone(),
                upstream_io.clone(),
                downstream_io.clone(),
                DeviceSurfaces {
            ...
            // The screen (and anything observed on it) dies with this boot —
            // reset, power-off, or any other exit path.
            let _clear_on_exit = ClearOnExit(observation.clone());

            let mut rebooting = false;
            {
                match device.session() {
                    InitOutcome::Ready(mut session) => {
                        // This boot's identity: the same one as last boot when the header
                        // survived (power-cycle, restore), a NEW one when the header was
                        // re-inited (the boot after an erase).
                        identity.publish(session.device_id());
                    ...
                    // Recovery erase ran on boot: reboot into the erased flash.
                    InitOutcome::ResetRequested => rebooting = true,
                }
            }

            // Session dropped (loop/UI/HAL torn down); preserve the flash for
            // the next boot.
            flash = device.into_flash();
            if !rebooting || thread_stop.load(Ordering::Relaxed) {
                break;
            }
        }
        flash
    ...
    rx.recv()
        .expect("device thread should boot and publish its id");
...
impl VirtualDevice {
    ...
    pub fn spawn(
        ...
        let observation = SimObservation::new();
        ...
        SpawnedDevice {
            device_id: identity.get().expect("the first boot published its id"),
            framebuffer,
            touch,
            observation,
            host: Some(host),
