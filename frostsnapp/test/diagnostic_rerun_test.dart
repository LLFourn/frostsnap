...
import '../test_driver/test_outcome.dart' show statusIsFailure;
...
void main() {
  ...
  group('selectDiagnosticReruns', () {
    ...
    // The runner's own predicate, so this pins what `--record-failures` actually selects.
    bool isFinalFailure(({String status, String name}) r) =>
        statusIsFailure(r.status);
    ...
    test('selects only final failures (incl. timeouts), in batch order', () {
      ...
      expect(selected.map((r) => r.name), ['b', 'd', 'e', 'g']);
    });

    test('an xfail is not re-run, an XPASS is', () {
      // An xfail failed exactly where it declared it would: recording a video of it is noise, and
      // on android it costs an emulator boot. A stale expectation IS a failure worth recording.
      final selected = selectDiagnosticReruns(results, true, isFinalFailure);
      expect(selected.map((r) => r.name), isNot(contains('f')));
      expect(selected.map((r) => r.name), contains('g'));
    });
