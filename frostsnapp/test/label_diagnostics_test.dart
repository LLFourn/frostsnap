  ...
  group('composite diagnosis wiring', () {
    test(
      ...
      () async {
        ...
        // Exactly ONE probe: the classifier's. The composite's own RAW snapshot fetch never runs
        // here — with the transport dead the wrapper fails on its frame-sync phase, before the
        // body — so this counts one where it used to count two. The property under test is
        // unchanged and still exact: a diagnosed inner transport would classify the same failure
        // again and push this higher.
        expect(driver.requestDataCalls, 1);
