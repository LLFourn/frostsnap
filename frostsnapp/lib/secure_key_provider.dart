...
abstract class SecureKeyProvider {
  ...
  /// Convenience accessor for the app encryption key from the global instance.
  ///
  /// The Android side heals a permanently-invalidated key in place (deletes the
  /// dead alias and recreates it), so this always yields a working key or throws
  /// a genuine platform failure. A wallet that was under the old key then reads
  /// as wrong-key and drives per-wallet recovery.
  static Future<SymmetricKey> getEncryptionKey() => instance.getOrCreateKey();
}
