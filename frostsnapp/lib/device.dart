...
class _DeviceDetailsState extends State<DeviceDetails> {
  ...
  void showEraseDialog(BuildContext context, DeviceId id) async {
    ...
    controller = FullscreenActionDialogController<void>(
      ...
      // Disable back-gesture: `onDismissed: null` flips `PopScope.canPop` to
      // false, so back invokes `showCannotDismissDialog` instead of popping
      // then having the controller immediately reshow (flicker). User must
      // hit Cancel or wait for the erase to complete.
      onDismissed: null,
    );
    ...
    try {
      ...
      // Only show the success dialog + pop DD if the user actually confirmed
      // the erase. If they canceled (or the dialog dismissed for some other
      // reason), leave them on DD.
      if (!_eraseConfirmed.value) return;

      if (context.mounted) {
        await _showEraseSuccessDialog(context, removedFromWallet);
      }
      // Device is gone for good — close DeviceDetails itself so the user isn't
      // left staring at the "waiting for device to connect" spinner.
      if (mounted) {
        final route = ModalRoute.of(context);
        if (route?.isCurrent == true) Navigator.pop(context);
      }
  ...
  Future<void> _showEraseSuccessDialog(
