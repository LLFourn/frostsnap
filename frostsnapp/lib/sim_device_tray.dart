...
/// The tray's mutation policy: run [op]; on failure surface the message via [onError]
/// AND converge via [refresh]. Both halves matter — an error without a refresh leaves a
/// stale list behind the message, a silent refresh makes the failed action look like it
/// worked. Top-level and dependency-injected so the policy itself is unit-testable.
Future<void> guardedTrayMutation(
  FutureOr<void> Function() op, {
...
class _SimTrayContentState extends State<SimTrayContent> {
  ...
  /// The last failed mutation's message, shown as a dismissible strip until the next
  /// successful mutation. Deterministic: set exactly when a mutation throws.
  ...
  Future<void> _refresh() async {
    ...
    // Read identity ONCE, here, while the handles are known good — never from `build`.
    // `number()`/`id()` are FRB calls into Rust, and a device removed since the last poll throws
    // `device N was removed`. Thrown during build that becomes a red ErrorWidget: the tray flashed
    // one on every removal, and nothing failed because the app simply carried on.
    // A device that dies DURING this refresh is dropped rather than carried into the frame.
    final snapshot = <_DeviceInfo>[];
    for (final device in devices) {
      try {
        snapshot.add(
          _DeviceInfo(device: device, number: device.number(), id: device.id()),
        );
      } catch (e) {
        // ONLY removal is expected here — the device went away between the pool read and now, so
        // it is simply not in this frame. Anything else means the router or the bridge is in a
        // state nobody intended, and swallowing it would show a silently missing card while the
        // suite reports green: the hidden-failure class this harness exists to end.
        if (!_isRemovedDevice(e)) rethrow;
      }
    }
    if (mounted) setState(() => _devices = snapshot);
  }
  ...
  void _mutationFailed(String message) {
    if (mounted) setState(() => _mutationError = message);
  }

  // A stale mutation (the device list changed under us — e.g. a CLI removal between
  // poll ticks) throws a clear error: show it AND refresh (guardedTrayMutation).
  ...
  Future<void> _addDevice() async {
    await guardedTrayMutation(
      () async {
        ...
        if (mounted) setState(() => _mutationError = null);
      },
      onError: _mutationFailed,
      refresh: _refresh,
    );
  ...
  // Remove an (unplugged) device from the fleet — its number is tombstoned, never reused.
  Future<void> _removeDevice(_DeviceInfo device) async {
    await guardedTrayMutation(
      () async {
        // The CAPTURED number, not a fresh FRB read: by the time this runs the handle may be dead,
        // and the pool reports an unknown number as a mutation error the tray already shows.
        await widget.pool.removeDevice(number: device.number);
        if (mounted) setState(() => _mutationError = null);
      },
      onError: _mutationFailed,
      refresh: _refresh,
  ...
  void _setConnected(_DeviceInfo? device, bool connected) {
    unawaited(
      guardedTrayMutation(
        () {
          device?.device.setConnected(connected: connected);
          setState(() => _mutationError = null);
        },
        onError: _mutationFailed,
        refresh: _refresh,
...
class _SimTrayShellState extends State<SimTrayShell>
  ...
  Widget build(BuildContext context) {
    ...
    // The duplicate-target fixture sits ABOVE the tray so its controls are genuinely hit-testable,
    // which is the whole point of it (see SimDuplicateTargets). Inert unless a test turns it on.
    return RepaintBoundary(
      key: simAppScreenshotKey,
      child: Stack(
        textDirection: TextDirection.ltr,
        children: [
          surface,
          const SimDuplicateTargetLayer(),
          const SimErrorProvokerLayer(),
        ],
      ),
    );
  }
...
/// The router's tombstone for a device the pool has removed — the one identity failure the tray
/// expects, because the pool can remove at any moment and the tray polls.
bool _isRemovedDevice(Object error) =>
    '$error'.contains(RegExp(r'device \d+ was removed'));

/// A device's identity as of the last poll, plus its handle.
///
/// The identity is CAPTURED, not read on demand: `number()` and `id()` are FRB calls that throw
/// once the device is removed, and a throw from inside `build` becomes a red ErrorWidget. The
/// handle is still here for actions and frame streaming, which happen outside build.
class _DeviceInfo {
    ...
    return _Card(
      ...
      child: Row(
        children: [
          ...
          IconButton(
            onPressed: onRemove,
            tooltip: 'Remove device ${device.number}',
            visualDensity: VisualDensity.compact,
            icon: Icon(
...
class _DeviceScreenState extends State<_DeviceScreen> {
  ...
  void initState() {
    super.initState();
    // onError is REQUIRED, not defensive: the frame stream belongs to a device the pool can remove
    // under us, and it then errors with `device N was removed`. Without a handler that escapes into
    // the zone as an uncaught async error — the same use-after-removal as the build-time reads,
    // just on the other side of the async boundary. A card whose device is gone simply stops
    // updating; the tray drops it on the next poll.
    // No onError swallow: `frames` now CLOSES normally for a removed device (see sim.rs), so any
    // error arriving here is a real stream fault, and letting it reach the zone is what makes the
    // detector see it. Consuming every error would trade a red test for a frozen screen.
  ...
  void _pointerDown(Offset local, Size rendered) {
    ...
    _downX = x;
    _downY = y;
    _inject(() => widget.device.touch(x: x, y: y, liftUp: false));
  }

  // A pointer stroke can race a removal (the screen vanishes within a poll tick);
  // touching a just-removed device's glass is inert, not an error surface.
  // Future.sync catches both sync throws (touch) and async ones (swipe).
  void _inject(FutureOr<void> Function() op) {
    unawaited(Future.sync(op).catchError((_) {}));
  }
  ...
  void _pointerUp(Offset local, Size rendered) {
    ...
    _inject(() => widget.device.touch(x: x, y: y, liftUp: true));
  }
