...
Future<void> main() async {
  ...
  try {
    ...
    if (kSim) {
      ...
      const compileSimRegtestElectrumBackup = String.fromEnvironment(
        'SIM_REGTEST_ELECTRUM_URL_BACKUP',
      );
      final simRegtestElectrumBackup =
          Platform.environment['SIM_REGTEST_ELECTRUM_URL_BACKUP'] ??
          compileSimRegtestElectrumBackup;
      simSettings = appCtx_.settings;
      if (simRegtestElectrum.isNotEmpty) {
        ...
        simRegtestControlSocket = simRegtestControl.isEmpty
            ? null
            : simRegtestControl;
        // Two distinct servers only when the harness supplied a second one; otherwise keep the
        // one-server shape every existing regtest test runs on.
        final hasBackup = simRegtestElectrumBackup.isNotEmpty;
        simRegtestElectrumBackupUrl = hasBackup
            ? simRegtestElectrumBackup
            : null;
        final regtest = BitcoinNetwork.fromString(string: 'regtest')!;
        // Seeded unconditionally, so a restart re-applies it: a test that repoints the servers
        // and then restarts gets these back.
        await appCtx_.settings.setElectrumServers(
          network: regtest,
          primary: simRegtestElectrum,
          backup: hasBackup ? simRegtestElectrumBackup : simRegtestElectrum,
        );
...
Widget buildMainWidget(AppCtx appCtx, Stream<String> logStream) {
  ...
  // Published so the sim's app channel reads status through the SAME per-network subject the UI
  // does. `subscribeChainStatus` installs a single-owner sink in the coordinator, so a second
  // subscription would displace the UI's and leave it deaf.
  if (kSim) simSettingsContext = settingsContext;
