...
pub(crate) fn build_device(
    ...
    SimDevice::new(number, index, device_id, frames_sink, router.clone())
}
...
impl DevicePool {
    ...
    /// make it stale afterwards with [`SimDevice::set_firmware_digest`]. Errors if the
    /// minted identity is already live (possible after restoring another session's
    /// device whose seed a fresh allocation would reuse).
