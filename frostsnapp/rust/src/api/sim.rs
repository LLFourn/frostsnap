...
use frostsnap_virtual_device::{
    ...
    SharedFramebuffer, SlotSpec, TouchEvent, TouchGesture, TouchQueue,
};
...
impl SimDevice {
    ...
    pub fn record_backup(&self) -> anyhow::Result<String> {
        Ok(record_on_device(
            &DeviceInput::new(self.touch.clone()),
            &self.router.observation(self.number as usize - 1),
    ...
    pub fn backup_display_next(&self) {
        DeviceInput::new(self.touch.clone()).backup_display_next();
    }
    ...
    pub fn backup_display_confirm(&self) {
        DeviceInput::new(self.touch.clone()).backup_display_confirm();
    }
    ...
    pub fn displayed_backup(&self) -> anyhow::Result<String> {
        self.router
            .observation(self.number as usize - 1)
            .displayed_backup()
