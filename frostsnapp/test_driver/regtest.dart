...
/// Front door `b`'s url — the app's BACKUP slot — read off the control socket. Deliberately not
/// added to `regtest.url`: that file is the readiness signal callers read WITHOUT connecting, and
/// the doors are already the socket's business.
Future<String> _backupElectrumUrl(String controlSocket) async {
  final faucet = await SimFaucet.connect(controlSocket);
  try {
    final servers = await faucet.electrumServers();
    final backup = servers.firstWhere(
      (s) => s['id'] == 'b',
      orElse: () => throw StateError('regtest backend has no front door `b`'),
    );
    return backup['url'] as String;
  } finally {
    await faucet.close();
  }
}
...
class RegtestSession {
  /// The electrum URL (`tcp://127.0.0.1:<dynamic-port>`) for this session's chain — front door
  /// `a`, which the app's PRIMARY slot points at.
  final String url;

  /// Front door `b`, for the app's BACKUP slot. A different HOST from [url] (`localhost` against
  /// `127.0.0.1`) so the two get separate TOFU trust entries — the trust store is keyed by host
  /// alone.
  ...
  Map<String, String> get hostDefines => {
    ...
    'SIM_REGTEST_ELECTRUM_URL_BACKUP': backupUrl,
...
Future<RegtestSession> startRegtestSession(Directory dir) async {
  ...
  try {
    ...
    return RegtestSession._(
      ...
      backupUrl: await _backupElectrumUrl(controlSocket),
...
const _bridgeElectrumBackupPort = 53323;
...
/// The BACKUP slot's bridged endpoint. `localhost` rather than `127.0.0.1` for the same reason the
/// host doors differ: TOFU keys its trust store by host alone, so the app must see two names or
/// the two servers share one trust entry. Both resolve to emulator loopback.
const androidBridgeElectrumBackupUrl =
    'tcp://localhost:$_bridgeElectrumBackupPort';
...
Future<RegtestEmulatorBridge> bridgeRegtestToEmulator(
  ...
  for (final (bridged, target) in [
    (_bridgeElectrumPort, electrumPort(session.url)),
    (_bridgeElectrumBackupPort, electrumPort(session.backupUrl)),
  ]) {
