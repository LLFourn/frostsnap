...
| `session.scrollIntoView(label)` | `void` | scroll the nearest scrollable until the (already built) widget matching `label` is on screen |
...
| `session.getSelectableTextByKey(key)` | `String` | `getTextByKey` for a `SelectableText`, which `getTextByKey` cannot read |
...
| `.tooltips()` | `List<String>` | onstage tooltips in traversal order — for state a widget shows only in its tooltip (e.g. the chain-status icon's `"<state>: <url>"`) |
...
### Electrum — the app's two servers

The regtest chain is served through two *front doors* the harness controls, not one endpoint.
Door `a` is the app's primary slot and door `b` its backup; they differ in HOST (`127.0.0.1`
against `localhost`) because TOFU keys its trust store by host alone, so two doors on one name
would share a trust entry. Drive the doors via `session.faucet()` above; read what the app made
of it here.

| call | returns | does |
|------|---------|------|
| `.electrumStatus()` | `ElectrumStatus` | the app's own view: `state`, `onBackup`, `primaryUrl`, `backupUrl` |
| `.waitForElectrum(pred, describe: ..)` | `ElectrumStatus` | poll until `pred` holds, or fail saying what it was |
| `.setElectrumServers(primary:, backup:)` | `void` | repoint the slots (settings API, not the Edit Server dialog) |
| `.setElectrumEnabled(enabled)` | `void` | `all` / `primary_only` / `none` |

Assert `electrumStatus`, not consequences. "The balance eventually appeared" passes just as
well when no failover happened.

To ask whether the app *reconnected*, compare a door's `accepted` before and after. The live
`connections` count cannot answer it — a kept connection and a torn-down-then-remade one both
end at one — and sampling for the gap between them is a race the test loses.

```dart
final faucet = await session.faucet();
final servers = await faucet.electrumServers();               // [{id: a, ..}, {id: b, ..}]
await faucet.electrumSetMode('a', 'down');                    // primary stops answering
await app.waitForElectrum(
  (s) => s.connectedTo(backup: true),
  describe: 'connected to the backup',
);
```

A door in `tls` mode presents a NAMED identity, minted once per session and byte-stable after
that (TOFU change-detection is a byte comparison, so a re-minted cert would read as a rotation
on every reconnect). `shape` picks how a new name is minted — `normal`, `expired`, `wrong_host`,
or `signed_by` (with `issuer`, for a leaf chaining to another identity). Reusing a name for a
different shape, or on the other door, is refused rather than silently serving the wrong cert.

```dart
final url = await faucet.electrumSetMode('a', 'tls', identity: 'a1');   // ssl://127.0.0.1:P
await faucet.electrumSetMode('a', 'tls', identity: 'a2');               // a rotation, to the app
```
...
| `.electrumUrl()` | `String` | front door `a`'s url — what the app's PRIMARY slot points at |
| `.electrumServers()` | `List<Map>` | both front doors: `id`, `url`, `mode`, `identity`, `fingerprint`, `connections` (live), `accepted` (cumulative) |
| `.electrumSetMode(id, mode, {identity, shape, issuer})` | `String` | put door `id` in `tcp`/`tls`/`down`/`hang`; returns its url |
| `.electrumDropConnections(id)` | `int` | kill door `id`'s live sessions, leaving it open; returns how many |
