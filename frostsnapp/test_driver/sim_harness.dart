...
import 'regtest.dart'
    show
        ...
        androidBridgeElectrumBackupUrl,
        androidBridgeElectrumUrl,
        androidSdkRoot,
        bridgeRegtestToEmulator,
        startRegtestSession;

/// The app's own view of its chain connection, as [AppSession.electrumStatus] reports it.
class ElectrumStatus {
  final String state;
  final bool onBackup;
  final String primaryUrl;
  final String backupUrl;

  ElectrumStatus._(Map<String, dynamic> json)
    : state = json['state'] as String,
      onBackup = json['onBackup'] as bool,
      primaryUrl = json['primaryUrl'] as String,
      backupUrl = json['backupUrl'] as String;

  bool get isConnected => state == 'connected';

  /// Connected, and to the slot the caller means.
  bool connectedTo({required bool backup}) => isConnected && onBackup == backup;

  @override
  String toString() =>
      'ElectrumStatus($state, onBackup: $onBackup, primary: $primaryUrl, backup: $backupUrl)';
}
...
class AppSemanticsInspector {
  ...
  /// On-stage tooltips, in traversal order. What a widget is CURRENTLY showing, for surfaces whose
  /// state lives in a tooltip rather than a label — the chain-status icon reads `"<state>: <url>"`,
  /// which is the only way to see what the app's OWN status subscriber last received.
  Future<List<String>> tooltips() async => [
    for (final node in await _nodes())
      if ((node['tooltip'] as String?)?.isNotEmpty ?? false)
        node['tooltip'] as String,
  ];
    ...
    return _ScenarioRegtest(
      ...
      _isHost(device)
          ...
          : {
              ...
              'SIM_REGTEST_ELECTRUM_URL_BACKUP': androidBridgeElectrumBackupUrl,
...
class AppSession {
  ...
  static Future<String> ensureAndroidSimApkBuilt({
    ...
    final proc = await Process.start('flutter', [
      ...
      '--dart-define=SIM_REGTEST_ELECTRUM_URL_BACKUP=$androidBridgeElectrumBackupUrl',
  ...
  /// Scroll the nearest scrollable until the control matching [label] is on screen. The control
  /// must already be BUILT: a lazy list that has not laid it out yet has nothing to scroll to.
  ///
  /// A merged label (a ListTile's title + subtitle) matches every widget that contributes to it,
  /// and scrolling needs exactly one, so this takes the first.
  Future<void> scrollIntoView(Pattern label) => _driverCall(
  ...
  /// [getTextByKey] for a `SelectableText`, which `getText` does not support and whose text is a
  /// semantics VALUE rather than a label. Reads the `EditableText` it builds.
  Future<String> getSelectableTextByKey(String key) => _driverCall(
    () => driver.getText(
      find.descendant(
    ...
    verb: 'getSelectableTextByKey("$key")',
    effect: DriverEffect.observes,
  );
  ...
  /// What the app believes about its regtest chain connection: `state`
  /// (`idle`/`connecting`/`connected`/`disconnected`), `onBackup`, and the two configured urls.
  ///
  /// The app's own view, not a widget's. Asserting a failover by its consequences — "the balance
  /// eventually appeared" — passes just as well when no failover happened.
  Future<ElectrumStatus> electrumStatus() async => ElectrumStatus._(
    jsonDecode(
          await _requestData('electrum-status', effect: DriverEffect.observes),
        )
        as Map<String, dynamic>,
  );

  /// Wait until the app's chain status satisfies [predicate], or fail describing what it was
  /// instead. Reconnects run on a backoff, so every failover assertion is a wait rather than a
  /// read.
  Future<ElectrumStatus> waitForElectrum(
    bool Function(ElectrumStatus) predicate, {
    ...
    var unanswered = 0;
    while (DateTime.now().isBefore(deadline)) {
      // Each READ is bounded, not just the loop. A deadline checked only between polls is no
      // deadline at all: one read that never returns parks here forever, and the caller's
      // `timeout:` becomes decoration. (Cost me a 700s hang to learn.)
      try {
        last = await electrumStatus().timeout(const Duration(seconds: 10));
        unanswered = 0;
      } on TimeoutException {
        unanswered++;
        continue;
    ...
    throw StateError(
      'electrum never became $describe within ${timeout.inSeconds}s '
      '(last: ${last ?? 'never read'}'
      '${unanswered > 0 ? '; $unanswered consecutive reads went unanswered' : ''})',
    );
  }

  /// Repoint the app's regtest electrum slots. The settings API, not the Edit Server dialog —
  /// a test that tapped its way through the dialog to move a url would be testing the dialog.
  Future<void> setElectrumServers({
    required String primary,
    required String backup,
  }) => _requestData(
    'electrum-set-servers:${jsonEncode({'primary': primary, 'backup': backup})}',
    effect: DriverEffect.mutates,
  );

  /// Which of the app's regtest slots may be connected to: `all`, `primary_only` or `none`.
  Future<void> setElectrumEnabled(String enabled) => _requestData(
    'electrum-set-enabled:$enabled',
    effect: DriverEffect.mutates,
  );
