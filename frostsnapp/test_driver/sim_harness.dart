...
class AppSession {
  ...
  Future<void> restartApp() async {
    ...
    await _runtime.restart(() async {
      ...
      final nextNumber = int.parse(await _requestData('next-device-number'));
      ...
      await _requestData('seed-next-number:$nextNumber');
  ...
  Future<void> removeDevice(int n) => _requestData('remove-device:$n');
  ...
  Future<String> saveDeviceState(int n) => _requestData('save-device-state:$n');
  ...
  Future<int> addDeviceFromSavedState(String state) async =>
      int.parse(await _requestData('add-device-from-saved-state:$state'));
  ...
  Future<String> staleDeviceHandleProbe(int n) =>
      _requestData('stale-handle-probe:$n');
  ...
  Future<String> _rawRequestData(String message) async {
    ...
    try {
      return await _boundedBySilentClock(
        driver.runUnsynchronized(() => driver.requestData(message)),
        budget: _cmdTimeout,
  ...
  Future<String> _requestDataTimeout(String message, Duration timeout) async {
    try {
      return await driver
          .runUnsynchronized(
            () => driver.requestData(message, timeout: timeout),
          )
          .timeout(timeout);
    } catch (original) {
      throw await _diagnosed(
  ...
  Future<String> walletDescriptor() => _requestData('wallet-descriptor');
  ...
  Future<int> showPsbtQr(String psbt) async =>
      int.parse(await _requestData('psbt-qr:$psbt'));

  /// Empty the sim camera's view, so a later scan can't re-decode the last [showPsbtQr].
  Future<void> hideQr() => _requestData('psbt-qr-clear');
