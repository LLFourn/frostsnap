...
//
//   fsim serve [--devices N]    launch the app + N devices ONCE (0 = start empty), listen for commands
...
fsim — drive the running sim app/devices through SimHarness.
  ...
  fsim serve [--devices N] [--instances N] [--android] [--platform <d>] [--agent-owns-keyboard] [--no-regtest]
                                              ...
                                              --devices N is optional sugar — 0 starts EMPTY, and the
                                              fleet is runtime-composable either way: addDevice /
                                              removeDevice at any time (numbers are permanent — a
                                              removed number is never reused);
  ...
  fsim test [NAMES...] [--android] [--jobs N] [--retries N] [--slot-wait SECS] [--record-failures] [--test-timeout SECS] [--nocapture|-v] [--junit PATH]
                                              ...
                                              a test may declare ONE assertion expected-to-fail
                                              (see COMMANDS.md): that reports `xfail` and does not
                                              fail the run, while an expectation that stops holding
                                              reports XPASS and DOES.
...
Common:
  ...
  await session.addDevice() / removeDevice(n)   grow / shrink the fleet at runtime (numbers permanent)
  await session.saveDeviceState(n)         device n's durable state as opaque base64 (must be disconnected)
  await session.addDeviceFromSavedState(s) restore a saved device as a new fleet member (same identity)
  await session.restartApp()               kill + relaunch the app on the SAME db (proves restore-from-db)
  ...
  await session.device(n).recordBackup()            record a whole backup: capture + page + hold; returns the text
  await session.device(n).displayedBackup()         the displayed backup's full text (sim eyes)
  await session.device(n).backupDisplayNext() / .backupDisplayConfirm()   drive the paged display
  await session.device(n).typeBackup('#1 WORD ...')  type a whole backup as real touches (~1 min)
...
class _TestResult {
  ...
  /// Failed exactly where it declared it would — reported, but not a failure of the run.
  bool get xfailed => status == 'XFAIL';

  /// Declared an expectation that no longer holds. A HARD failure: either the assertion
  /// started passing (delete the marker) or the run never reached it.
  bool get xpassed => status == 'XPASS';

  /// Whether this result counts against the run.
  bool get isFailure => statusIsFailure(status);
}
...
/// Clear a test's artifacts, PRESERVING the last failure.
///
/// A rerun is the first thing anyone does after a flake — and it used to delete the evidence for
/// the failure they are trying to explain, so an intermittent failure was diagnosable only if you
/// happened to read the artifacts before running anything again. The previous failure is moved
/// aside instead, and only ever replaced by another failure.
Future<void> _clearArtifacts(Directory dir) async {
  if (!await dir.exists()) return;
  final wasFailure = await File('${dir.path}/error.txt').exists();
  if (wasFailure) {
    final kept = Directory('${dir.path}-last-failure');
    if (await kept.exists()) await kept.delete(recursive: true);
    await dir.rename(kept.path);
    return;
  }
...
/// The first line of [output] carrying [marker] — the scenario's own words about its expectation,
/// used verbatim as the result's reason instead of a generic 'exit code 0'.
String? _markerLine(String output, String marker) {
...
void _printSummary(List<_TestResult> results, Duration elapsed) {
  ...
  final xfailed = results.where((r) => r.xfailed).length;
  final xpassed = results.where((r) => r.xpassed).length;
  ...
  stdout.writeln(
    ...
    '${xfailed > 0 ? ' ($xfailed xfail)' : ''}'
    '${xpassed > 0 ? ' ($xpassed XPASS — expectation is stale)' : ''}'
...
Future<void> _writeJunit(
  ...
  // An XPASS is a failure in the JUnit sense too: the expectation it declared is stale.
  final failed = results.where((r) => r.failed || r.xpassed).length;
  final timedOut = results.where((r) => r.timedOut).length;
  final skipped = results.where((r) => r.skipped || r.xfailed).length;
  ...
  for (final r in results) {
    ...
    if (r.skipped) {
      ...
      b.writeln(
        '    <skipped message="xfail: ${_xmlEscape(r.reason ?? 'expected failure')}"/>',
      );
    } else if (r.failed || r.xpassed) {
...
Future<void> _recordFailedTests(
  ...
  // Only what counts against the run: an xfail failed exactly where it declared it would, so
  // recording a video of it is noise (and, on android, a wasted emulator boot).
  final reruns = selectDiagnosticReruns(results, enabled, (r) => r.isFailure);
...
Future<_TestResult> _runOneTest(
  ...
  if (status == 'XFAIL') {
    reason =
        _markerLine(output, simTestExpectedFailHitMarker) ?? 'expected failure';
  } else if (status == 'XPASS') {
    reason =
        'the expectation is stale — '
        '${_markerLine(output, simTestExpectedFailDeclaredMarker) ?? 'declared an expected failure'}'
        ' — but it did not happen. If it is FIXED, retire the expectation in the same '
        'change: delete the ExpectedFailure, the expectedToFail: argument, the guard '
        'wrapper and its hit-return, leaving the assertion inline (see the retirement '
        'instructions in test_driver/COMMANDS.md). If the run never reached the guard, '
        'that is the failure.';
  }
  if (status != 'PASSED') {
...
Future<void> _captureVmDeviceFrames(Directory dir, String output) async {
  ...
  try {
    ...
    // Plain requests, NOT `runUnsynchronized`: this runs against an app that has just failed, and
    // the frame-sync round trips it adds are two more chances to hang before the artifact is saved.
    final csv = await driver
        .requestData('device-numbers')
        .timeout(_diagnosticTimeout);
    final numbers = csv.isEmpty ? <int>[] : csv.split(',').map(int.parse);
    for (final n in numbers) {
      try {
        final b64 = await driver
            .requestData('device-screen:$n')
            .timeout(_diagnosticTimeout);
