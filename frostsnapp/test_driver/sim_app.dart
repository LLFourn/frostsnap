...
/// The app's settings, published by the sim entrypoint. Absent means this is not a sim build,
/// which is a harness wiring error rather than anything the test did.
Settings _simSettings() {
  final settings = simSettings;
  if (settings == null) throw 'sim_app: no settings (not a sim build?)';
  return settings;
}
...
Future<String> _driverData(String? payload) async {
  ...
  // Repoint the regtest electrum slots, or change which of them are enabled, through the settings
  // API rather than the settings UI: a test that had to tap through Edit Server to move a url
  // would be testing that dialog on its way to testing failover. JSON-encoded because urls
  // contain the colon this dispatch splits on.
  const electrumServersPrefix = 'electrum-set-servers:';
  if (payload != null && payload.startsWith(electrumServersPrefix)) {
    final args =
        jsonDecode(payload.substring(electrumServersPrefix.length))
            as Map<String, dynamic>;
    await _simSettings().setElectrumServers(
      network: BitcoinNetwork.fromString(string: 'regtest')!,
      primary: args['primary'] as String,
      backup: args['backup'] as String,
    );
    return 'ok';
  }
  const electrumEnabledPrefix = 'electrum-set-enabled:';
  if (payload != null && payload.startsWith(electrumEnabledPrefix)) {
    final name = payload.substring(electrumEnabledPrefix.length);
    await _simSettings().setElectrumEnabled(
      network: BitcoinNetwork.fromString(string: 'regtest')!,
      enabled: switch (name) {
        'all' => ElectrumEnabled.all,
        'primary_only' => ElectrumEnabled.primaryOnly,
        'none' => ElectrumEnabled.none,
        _ =>
          throw 'sim_app: unknown electrum enabled "$name" '
              '(all|primary_only|none)',
      },
    );
    return 'ok';
  }
  ...
  switch (payload) {
    ...
    case 'psbt-qr-clear':
      ...
      return 'ok';
    case 'electrum-status':
      // What the APP believes about its chain connection, not what a widget renders: which
      // server it is on, and whether it is connected. A test that asserted only consequences
      // ("a balance appeared") would pass for the wrong reason on a failover it never made.
      // The UI's OWN stream, not a fresh subscription: subscribeChainStatus installs a
      // single-owner sink in the coordinator, so subscribing here would displace the UI's and
      // leave it deaf for the rest of the run. The shared subject replays its latest value, so
      // `first` is a snapshot rather than a wait.
      final context = simSettingsContext;
      if (context == null) {
        throw 'sim_app: no settings context (the app has not built yet?)';
      }
      final network = BitcoinNetwork.fromString(string: 'regtest')!;
      final status = await context
          .chainStatusStream(network)
          .first
          .timeout(const Duration(seconds: 10));
      return jsonEncode({
        'state': status.state.name,
        'onBackup': status.onBackup,
